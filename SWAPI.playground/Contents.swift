...
class SwapiService: Decodable {
    ...
    static func fetchPerson(id: Int, completion: @escaping (Person?) -> Void) {
        ...
        let mostFinalURL = finalURL.appendingPathComponent("\(id)")
        print(mostFinalURL)
        
        
        
        // 2 - Contact server
        URLSession.shared.dataTask(with: mostFinalURL) { (data, _, error) in
            // 3 - Handle errors
             if let error = error {
...
fetchFilm(url: URL(string: "https://swapi.co/api/films/5/")!)
